...
export const zoneHeader = (title, subtitle) => `
  <div class="zone-header">
    <h2 class="zone-title">${escapeHtml(title)}</h2>
    <p class="zone-subtitle">${escapeHtml(subtitle)}</p>
  </div>`;
export const allocationBars = (rows) => {
    if (!rows.length)
        return noteCard("No allocation data yet.", true);
    const bars = rows
        .map((row) => `
      <div class="alloc-row">
...
export const metricGrid = (items) => `<div class="metric-grid">${items.map(([label, value]) => metricCard(label, value)).join("")}</div>`;
export const noteCard = (text, quiet = false) => `<div class="note-card${quiet ? " note-card--quiet" : ""}">${escapeHtml(text)}</div>`;
export const insightChips = (items) => items
    .map(([label, tone]) => `<span class="insight-chip insight-chip--${escapeHtml(tone)}">${escapeHtml(label)}</span>`)
    .join("");
export const dataTable = (headers, rows) => {
    if (!rows.length)
        return noteCard("No rows to display.", true);
    return `
...
export const lineChart = (values, options) => {
    if (values.length < 2)
        return noteCard("Not enough data for a chart yet.", true);
    const width = 640;
    const height = 180;
    const padding = 18;
...
export const drawdownChart = (values) => {
    if (values.length < 2)
        return noteCard("Drawdown becomes visible after more weeks.", true);
    let peak = values[0];
...
export const portfolioInsightChips = (portfolio) => {
    const chips = [];
    const cashWeight = portfolio.total_nav > 0 ? portfolio.cash / portfolio.total_nav : 0;
    const maxWeight = portfolio.holdings.reduce((max, row) => Math.max(max, row.weight), 0);
    if (portfolio.positions === 0) {
        return [["No holdings yet", "neutral"], ["All capital is currently in cash", "good"]];
    }
    if (cashWeight > 0.65)
        chips.push(["Most capital is still in cash", "neutral"]);
    else if (cashWeight < 0.2)
        chips.push(["Most capital is currently invested", "good"]);
    if (maxWeight > 0.5)
        chips.push(["Portfolio is concentrated in one position", "risk"]);
    ...
    if (portfolio.nav_history.length >= 2) {
        ...
        if (portfolio.total_nav >= peak * 0.99)
            chips.push(["Portfolio value is near its visible high", "good"]);
        else
            chips.push(["Portfolio remains below its visible peak", "warn"]);
    }
    return chips;
};
export const marketPanel = (observation, selectedTicker) => {
    ...
    const change = selected && previousClose ? selected.close - previousClose : null;
    ...
    const weeklyChange = selected?.change_vs_previous_close !== null && selected?.change_vs_previous_close !== undefined
...
export const holdingsPanel = (portfolio) => {
    ...
    return dataTable(["Stock", "Shares Held", "Average Cost", "Current Market Value", "Portfolio Weight", "Active Stop"], rows);
};
export const pendingLiquidationsPanel = (items) => {
    if (!items.length) {
    ...
    return dataTable(["Stock", "Low That Triggered It", "Stop Price", "Scheduled Execution Week"], rows);
};
