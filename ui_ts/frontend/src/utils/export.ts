import type { SessionResponse } from "../api/client.js";

const slug = (value: string): string =>
  value.replace(/[^a-zA-Z0-9_-]+/g, "_").replace(/^_|_$/g, "") || "session";

const exportFilename = (session: SessionResponse, ext: string): string => {
...
export const buildSessionExport = (session: SessionResponse): Record<string, unknown> => ({
...
export const buildSessionCsv = (session: SessionResponse): string => {
  const lines: string[] = [];
  ...
  appendSection(
    ...
    "equity_curve",
    ["decision_step", "nav"],
    portfolio.nav_history.map((nav, index) => [index + 1, nav]),
  );

  appendSection(
    lines,
    "holdings",
    ["ticker", "shares", "average_cost", "market_value", "weight", "active_stop"],
  ...
  appendSection(
    ...
    "allocation",
    ["label", "value", "weight"],
    portfolio.allocation.map((row) => [row.label, row.value, row.weight]),
  );
...
export const downloadSessionJson = (session: SessionResponse): void => {
  downloadTextFile(
...
export const downloadSessionCsv = (session: SessionResponse): void => {
  downloadTextFile(
    exportFilename(session, "csv"),
